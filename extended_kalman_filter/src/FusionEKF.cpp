...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
    ...
    */
    ...
    if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
      ...
      */
      ...
      //px = rho*cos(phi), py= rho*sin(phi), vx=rho_dot*cos(phi), vy=rho_dot*sin(phi)
    ...
    // Store the timestamp as previous time
   ...
   */
  //calculate the time change in seconds
  float dt = (measurement_pack.timestamp_ - previous_timestamp_)/1000000.0;
  
  // to avoid repetitive caluculations
  float dt_2 = dt*dt;   // dt^2
  float dt_3 = dt*dt_2; // dt^3
  float dt_4 = dt*dt_3; // dt^4
