 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (unsigned int t = 0; t < N - 1; t++) {
      fg[0] += 70*CppAD::pow(vars[delta_start + t], 2);
      fg[0] += 25*CppAD::pow(vars[a_start + t], 2);
    }
    // Minimize the value gap between sequential actuations.
    for (unsigned int t = 0; t < N - 2; t++) {
      fg[0] += 700000*CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //put the steering and acceleration actuations first
  mpc_out.push_back(solution.x[delta_start]);
  mpc_out.push_back(solution.x[a_start]);
  //collect the x and y positions of the track
